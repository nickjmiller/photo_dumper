...
class ComparisonRepositoryImpl implements ComparisonRepository {
  ...
  @override
  Future<Either<Failure, ComparisonSession?>> getComparisonSession(
    String id,
  ) async {
    try {
  ...
  @override
  Future<Either<Failure, List<ComparisonSession>>>
  getComparisonSessions() async {
    try {
  ...
  Future<Either<Failure, ComparisonSession>> _mapModelToEntity(
    ComparisonSessionModel model,
  ) async {
    try {
      final allPhotoIds = model.allPhotoIds;
      final eitherPhotos = await photoRepository.getPhotosByIds(allPhotoIds);

      return eitherPhotos.fold((failure) => Left(failure), (photos) {
        final photoMap = {for (var p in photos) p.id: p};

        final allPhotosList = model.allPhotoIds
            .map((id) => photoMap[id])
            .whereType<Photo>()
            .toList();
        final eliminatedPhotosList = model.eliminatedPhotoIds
            .map((id) => photoMap[id])
            .whereType<Photo>()
            .toList();
        ...
        if (allPhotosList.length != model.allPhotoIds.length) {
          return Left(
            CacheFailure('Failed to reconstruct session: Photo data missing'),
          );
        }

        // Calculate remaining photos
        final eliminatedIds = eliminatedPhotosList.map((p) => p.id).toSet();
        final remainingPhotosList = allPhotosList
            .where((p) => !eliminatedIds.contains(p.id))
            .toList();
